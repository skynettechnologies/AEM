...
export const Home = () => {
    const [iframState,setIframeState]=useState('');
    useEffect(()=>{
        ...
        fetch('https://ada.skynettechnologies.us/check-website',options)
            ...
            .then(function(response) {
                ...
                if (response.status == 3) {
                    ...
                    iframeHtml +='<iframe src="https://ada.skynettechnologies.us/trial-subscription?isframe=true&website='+ webdomain +'&platform=aem&utm_source=aem-extension&utm_medium=create-account&utm_campaign=trial-subscription" height="1100px;" width="80%" style="border: none;"></iframe>';
                    iframeHtml +='</div>';
                }
                setIframeState(iframeHtml);
            })
            .catch((error) => {
    ...
    return (
        <View>
            <Heading level={1}>Welcome to All in One Accessibility!</Heading>
            <div dangerouslySetInnerHTML={{ __html: iframState }} />
        </View>
    )
